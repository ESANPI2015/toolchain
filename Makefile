...
# helper section:
#
# this will compile the exporter such that it fits to a certain
# representations-svn-revision of the istruct. so checking out the git to a
# certain date, recompiling everything and copying the binary to the name with
# the svn-revision appended.
#
# works only on git.
date_%:
	git checkout `git rev-list -n 1 --before="$*" master`
	make distclean all
	cp $(BUILDDIR)/protolog2csv_cmdline protolog2csv_cmdline_r`git log --name-status HEAD^..HEAD | perl -ne 'print $$1 if /\s?git-svn-id: [^@]*@(\d+) .*/'`
